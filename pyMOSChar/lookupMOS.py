...
def init(fileName='MOS.dat'):
    ...
    mosDat = pickle.load(open(fileName, 'rb'), encoding='bytes')
    mosDat = dict((k, v) for (k, v) in mosDat.items())
    mosDat['nfet'] = dict((k, v) for (k, v) in mosDat['nfet'].items())
    mosDat['pfet'] = dict((k, v) for (k, v) in mosDat['pfet'].items())
...
def lookup(mosType, *outVars, **inVars):
    ...
    if (mode == 1 or mode == 2):
        if len(mosDat[mosType]['length']) == 1:
            xi_mesh = np.array(np.meshgrid(VSB, VDS, VGS))
            xi = np.rollaxis(xi_mesh, 0, 4)
            xi = xi.reshape(xi_mesh.size/3, 3)
            points = points[1:]
            ydata = ydata[0,:,:,:]
            interpdata = interpn(points, ydata, xi, method='linear')
            result = np.squeeze(interpdata.reshape(len_VSB, len_VDS, len_VGS))
        else:
            interpdata = interpn(points, ydata, xi, method='linear')
            result = np.squeeze(interpdata.reshape(len_L, len_VSB, len_VDS, len_VGS))
            if np.isnan(np.sum(result)):
                print('Trying nearest interpolation method!')
                interpdata = interpn(points, ydata, xi, method='nearest')
                result = np.squeeze(interpdata.reshape(len_L, len_VSB, len_VDS, len_VGS))
    elif (mode == 3):
